...
// Same launch config as optimized (512 threads, 64x64 tiles) but simpler implementation
// No shared memory, no tensor cores - just direct global memory access
__global__ void sgemm_global_mem_coalesce(const float *A, const float *B, float *C) {
    ...
    constexpr int REG_M = 4;    // Each thread computes 4 rows
    constexpr int REG_N = 2;    // Each thread computes 2 cols
